...
namespace bookstoreagent.Workflow
{
    ...
    {
        public Microsoft.Agents.AI.Workflows.Workflow Build(string lastAgent)
        {
...
#pragma warning disable MAAIW001 // 类型仅用于评估，在将来的更新中可能会被更改或删除。取消此诊断以继续。
            var workflow = AgentWorkflowBuilder.CreateHandoffBuilderWith(lastAgent switch
            {
                BookRecommendationAgentBuilder.Name=>recommendationAgent,
                BookOrderAgentBuilder.Name=>bookOrderAgent,
                _=>throw new ArgumentException("unknow agent!")
            })
                .WithHandoff(recommendationAgent, bookOrderAgent, "The user selected a specific book and is ready to place an order ")
                .WithHandoff(bookOrderAgent,recommendationAgent,"The user wants to change the book ")
                .Build();
